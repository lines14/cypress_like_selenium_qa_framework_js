...
Cypress.Commands.add('waitIsExisting', { prevSubject: false }, (locator) => {
  const convertLocator = (document, xpath) => {
    const nodeList = [];
  ...
  return cy.waitUntil(
    () => cy.document().then((document) => {
      const nodes = convertLocator(document, locator);
...
Cypress.Commands.add('waitIsNotExisting', { prevSubject: false }, (locator) => {
  const convertLocator = (document, xpath) => {
    const nodeList = [];
    const result = document.evaluate(xpath, document, null, XPathResult.ANY_TYPE, null);
    let node; // eslint-disable-next-line no-cond-assign
