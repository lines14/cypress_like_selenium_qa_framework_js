...
class OnesAPI extends BaseAPI {
  ...
  async getActiveOGPOPolicies({
    ...
    Logger.log('[inf]   attempting active policies retrieval!');
    ...
    Logger.log(`[inf]   params: ${JSON.stringify(params)}`);

    // If request didn't succeed, retry.
    const response = await this.#API.post(JSONLoader.APIEndpoints.ones.callMethod, params);
    if (response.status !== 200) {
      Logger.log('[inf]   active policies request didn\'t succeed!');
      return this.getActiveOGPOPolicies({ attemptCountArg: attemptCount });
    }
    ...
    if (!filteredPolicies.length) {
      Logger.log('[inf]   no policies left after filtering! Attempting request again!');
      return this.getActiveOGPOPolicies({ attemptCountArg: attemptCount });
    }

    Logger.log('[inf]   active policies retrieval successfully finished!');
